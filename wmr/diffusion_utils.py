...
class DiffWMAttacker:
    def __init__(self, pipe, noise_step: int=60):
        self.pipe = pipe
    ...
    def attack(self, image):
        ...
        # img is a PIL image, convert to pytorch tensor
        img_ch = torch.tensor(np.array(img)).unsqueeze(0).permute(0, 3, 1, 2)
        img_ch = img_ch.float() / 255.
        # Make sure it is on same device as input
        img_ch = img_ch.to(self.device)
